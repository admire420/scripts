...
// // Consicutive number digit that are repeating in same position : 2358 & 4395 (3 is repeating in same position)
// const {
//    crd1,
//    crd2,
//    crd3,
...
//    crd134,
//    crd1234,
// } = findConsicutiveRepeatingDigits(WINNING_BIDS);
// let consicutiveRepeatingDigitPatterns = [
//    { name: "crd1", data: crd1 },
//    { name: "crd2", data: crd2 },
//    { name: "crd3", data: crd3 },
//    { name: "crd4", data: crd4 },
//    { name: "crd12", data: crd12 },
//    { name: "crd13", data: crd13 },
//    { name: "crd14", data: crd14 },
//    { name: "crd24", data: crd24 },
//    { name: "crd23", data: crd23 },
//    { name: "crd34", data: crd34 },
//    { name: "crd123", data: crd123 },
//    { name: "crd234", data: crd234 },
//    { name: "crd124", data: crd124 },
//    { name: "crd134", data: crd134 },
//    { name: "crd1234", data: crd1234 },
// ];
// console.log("");
// consicutiveRepeatingDigitPatterns.forEach((pattern) => {
//    console.log(
//       `Consicutive Repeating Digit - ${pattern.name} :` + pattern.data.length,
//       "| Total Probability :" +
...
// Consicutive number digits that are repeating in different positions and length
// bids, consicutive number shift ( 1 means today and yesterday(2))
{
   const REPEATING_INTERVAL = 1;
   const { data, filteredData } = findConsicutiveRepeating(
      WINNING_BIDS,
      REPEATING_INTERVAL
   );
   console.log("");
   console.log("Consicutive Repeating Pattern Interval: ", REPEATING_INTERVAL);
   filteredData.forEach((pattern) => {
      const name = pattern;
      const count = data[pattern].count;
      console.log(
         `${name} :` + count,
         "| Total Probability :" + ((count / totalBids) * 100).toFixed(2) + "%"
      );
   });
}
...
filteredData.forEach((pattern) => {
   ...
   const count = data[pattern].count;
   console.log(
      `Repeating Pattern - ${name} :` + count,
      "| Probability Repeating :" +
         ((count / totalRepeatingBids) * 100).toFixed(2) +
         "%",
      "| Total Probability :" + ((count / totalBids) * 100).toFixed(2) + "%"
   );
});
